Attribute VB_Name = "AO_VBA_LIB_060"
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
''
...
Function aoFindPatternUsingLoop( _
...
Public Function aoDeleteEmptyBottomRows(ws As Worksheet)
    Dim lastUsedRow As Long, nextRow As Long, underLastFiltered As Range, blankCells As Range
    lastUsedRow = ws.UsedRange.rows.Count
    
    Dim col As Range, lastRow As Long, maxRow As Long
    For Each col In ws.UsedRange.Columns
        lastRow = col.EntireColumn.rows(col.EntireColumn.rows.Count).End(xlUp).row
        
        If lastRow < lastUsedRow Then
            Set underLastFiltered = Range(col.cells(lastRow + 1, 1), col.cells(lastUsedRow, 1))
            'aoSelect underLastFiltered
            Set blankCells = aoSpecialCells(underLastFiltered, xlCellTypeBlanks)
            If blankCells Is Nothing Then
                lastRow = lastUsedRow
            Else
                'aoSelect blankCells
                lastRow = blankCells.Areas(blankCells.Areas.Count).cells(1, 1).row - 1
            End If
        End If
        
        If lastRow > maxRow Then
            maxRow = lastRow
            If maxRow = lastUsedRow Then
                Exit For
            End If
        End If
    Next
    
    If maxRow < ws.UsedRange.rows.Count Then
        Dim toDelete As Range, nonEmpty As Range
        Set toDelete = Range( _
            ws.UsedRange.cells(maxRow + 1, 1), _
            ws.UsedRange.cells(ws.UsedRange.rows.Count, ws.UsedRange.Columns.Count))
        
        'aoSelect toDelete
        Set nonEmpty = aoFind(toDelete, "*")
        If Not (nonEmpty Is Nothing) Then
            Err.Raise vbObjectError + 513, ws.name & " - " & toDelete.Address, "aoDeleteEmptyBottomRows failed"
        End If
...
Function aoSheet( _
    ...
    Set aoSheet = aoSheetLike(wb, name)
    If aoSheet Is Nothing Then
        ...
        If index = 0 Then
            ...
            Err.Raise vbObjectError + 513, "Error in" & wb.name, "Zero-based index for aoSheet"
        End If
        
        ' -1 means last sheet, ala Perl:
        While index < 1
            index = index + wb.Sheets.Count + 1
        Wend
        
        If index <= wb.Sheets.Count Then
            Set aoSheet = wb.Sheets(index)
        End If
    End If
